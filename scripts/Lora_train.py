...
class FixedMushroomDataset(Dataset):
    ...
    def find_image_path(self, label_name, filename, original_path):
        base_path = "/zfs/ai4good/datasets/mushroom/merged_dataset"
        path = os.path.join(base_path, label_name, filename)
        return path
...
#main loop
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader, 1):
            ...
            current_temperature = adaptive_temperature(epoch, total_loss / max(1, processed_batches))
            
            loss = stable_cross_entropy(logits / current_temperature, labels, LABEL_SMOOTHING)
            ...
            #performs gradient descent with adaptive lr
            if (batch_idx % accumulation_steps == 0) or (batch_idx == len(train_loader)):
                current_lr = adaptive_lr(epoch, batch_idx, len(train_loader), loss.item() * accumulation_steps)
                ...
                if text_features.grad is not None:
                    text_grad_norm = torch.nn.utils.clip_grad_norm_([text_features], max_norm=0.1)
            ...
            if batch_idx % 20 == 0:
                ...
                print(f"Batch {batch_idx:3d}/{len(train_loader)} | Loss: {loss.item()*accumulation_steps:.4f} | Avg: {avg_loss:.4f}")
                
                with open(acc_log_path, "a") as f:
                    f.write(f"{epoch},{batch_idx},{loss.item()*accumulation_steps:.6f},,,,,,{current_lr:.2e},{current_temperature:.4f},0.0\n")
        #summary of epoch
